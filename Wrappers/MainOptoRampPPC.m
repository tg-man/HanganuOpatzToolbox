...
ch2load = 19;
save_data = 1;
folder2save = 'Q:\Personal\Tony\Analysis\Results_RampPPC\';
folder4matrix = 'Q:\Personal\Tony\Analysis\Results_OptoMatricesRamp_shift\';
...
%     LFP(bad_ch - 16, :) = NaN; % Do the same here for noisy channel 
...
%     
    ...
    for area_idx = 1 : size(areas,2)        
        ...
        if (strcmp(area, 'TH') && experiment.target3 == 1) || (strcmp(area, 'Str') && experiment.target2 == 1)    
            disp(['computing PPC for ' area])
            % load spike matrix 
            ...
            for freq_idx = 1 : length(freqs) - 1
                ...
                for ramp = 1 : size(spike_matrix, 1)
                    LFPramp = LFPfilt(stim_end(ramp) - 5999: stim_end(ramp) + 4000);  
                     for unit = 1 : size(spike_matrix, 2)
                         ...
                         phase = angle(hilbert(LFPramp)); % basically a getPhaseLocking function
                ...
                PLV_stim = squeeze(PLV(freq_idx, :, :, 3001:6000)); 
                ...
                PLV_pre = reshape(PLV_pre, [], unit, 1);
                PLV_stim = reshape(PLV_stim, [], unit, 1); 
            ...
            PhaseStuff.Notes = [' PLV: dimensions: freq_idx, ramp, unit, phase@spike time' ...
                'PPC: freq_idx, unit']; 
...
figure; 
violinplot(PPCstrplot, x, 'ViolinAlpha', 0.7, 'Width', 0.35, 'ShowData', false); 
ylabel('PPC'); xlabel('freq bands'); xticklabels({'4-12 pre', '4-12 stim', '12-30 pre', '12-30 stim', '30-70 pre', '30-70 stim'}); 
...
figure; 
violinplot(PPCthplot, x, 'ViolinAlpha', 0.7, 'Width', 0.35, 'ShowData', false); 
ylabel('PPC'); xlabel('freq bands'); xticklabels({'4-12 pre', '4-12 stim', '12-30 pre', '12-30 stim', '30-70 pre', '30-70 stim'}); 
