...
experiments = experiments(360:380);
...
folder4osc = 'Q:\Personal\Tony\Analysis\Results_osc\'; %Fill in path where to save oscillations
folder4PSD = 'Q:\Personal\Tony\Analysis\Results_PSD\'; %Fill in path where to save PSD
...
for exp_idx = 1 : size(experiments, 2)
    ...
    if experiment.target1 == 1
        osc_PFC = getOscillations(experiment, LFP4osc_PFC, fs_LFP, freq_band, rel_thresholds, ...
            abs_thresholds, threshold_artifact, durations, save_data, repeatCalc, [folder4osc experiment.Area1 '\'], 1, verbose);
    end 
    if experiment.target2 == 1
        osc_Str = getOscillations(experiment, LFP4osc_Str, fs_LFP, freq_band, rel_thresholds, ...
            abs_thresholds, threshold_artifact, durations, save_data, repeatCalc, [folder4osc experiment.Area2 '\'], 1, verbose);
    end 
    if experiment.target3 == 1
        osc_TH = getOscillations(experiment, LFP4osc_TH, fs_LFP, freq_band, rel_thresholds, ... 
            abs_thresholds_TH, threshold_artifact, durations, save_data, repeatCalc, [folder4osc experiment.Area3 '\'], 1, verbose); 
    end 
    ...
    if experiment.target1 == 1 
        PSD_PFC = getPSD_across_timescales(LFP_PFC(~isnan(LFP_PFC(:,1)),:), fs_LFP, freq2analyze, slow_freqs, fast_freqs, ...
            [folder4PSD experiment.Area1 '\'], experiment.animal_ID, save_data);
    end 
    if experiment.target2 == 1 
        PSD_Str = getPSD_across_timescales(LFP_Str(~isnan(LFP_Str(:,1)),:), fs_LFP, freq2analyze, slow_freqs, fast_freqs, ...
            [folder4PSD experiment.Area2 '\'], experiment.animal_ID, save_data);
    end 
    if experiment.target3 == 1
        PSD_TH = getPSD_across_timescales(LFP_TH(~isnan(LFP_TH(:,1)),:), fs_LFP, freq2analyze, slow_freqs, fast_freqs, ...
            [folder4PSD experiment.Area3 '\'], experiment.animal_ID, save_data);
    end 
