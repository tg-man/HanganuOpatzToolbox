...
experiments = experiments([300 301 324:426]);  % [300 301 324:380]
save_data = 1; 
    ...
    if [experiments4mouse.(['target' num2str(find(strcmp({'ACC','Str','TH'}, BrainArea)))])],(1) == 1; % check targetting 
        ...
        for exp_idx = 1 : size(experiments4mouse, 2) 
            ...
            if double(string(Calls.('Type')(1))) == 9 && double(string(Calls.('Type')(end))) == 8  
                ...
                types = double(string(Calls.('Type')(2:end-1))); 
            else 
            ...
            if size(syllables, 1) > 1
                bursts = []; 
                last = 0; 
                bursts(1).start = syllables(1,1); 
                bursts(1).stop = syllables(1,2); 
                for sy_idx = 2 : size(syllables, 1) 
                    if syllables(sy_idx, 2) - bursts(end).stop < minInterSyInt
                        bursts(end).stop = syllables(sy_idx, 2);
                    else 
                        bursts(end).type = types(last + 1 : sy_idx - 1); 
                        bursts(end + 1).start = syllables(sy_idx, 1);
                        bursts(end).stop = syllables(sy_idx, 2);
                        last = sy_idx - 1; 
                    end 
                end
                bursts(end).type = types(last + 1 : sy_idx); 
    
                % in case the first one starts too early, drop 
                while bursts(1).start < minInterSyInt
                    bursts(1) = []; 
                end 
                % in case the last call is too late, drop
                while bursts(end).stop + minInterSyInt > size(spike_matrix,2)
                    bursts(end) = [];  
                end  

                % filter out based on call types 
                keep = []; 
                for burst_idx = 1 : size(bursts, 2)
                    if calltype == 1 && mean(bursts(burst_idx).type == calltype) > 0.8
                        keep(burst_idx) = 1; 
                    elseif calltype == 2 && mean(bursts(burst_idx).type == calltype) > 0.8
                        keep(burst_idx) = 1; 
                    elseif calltype == 3 && mean(bursts(burst_idx).type == calltype) > 0.5
                        keep(burst_idx) = 1; 
                    end 
                end 
                bursts = bursts(logical(keep)); 

                % make plotting matrix, cells X (double song inter) X trials 
                if size(bursts, 2) >= 1 % check if there're enough songs 
                    % initialize plotting matrix
                    usvmat = zeros(numel(clusters), 2*minInterSyInt, size(bursts, 2)); 
                    for song_idx = 1 : size(bursts, 2)
                        usvmat(:, :, song_idx) = full(spike_matrix(:, (bursts(song_idx).start - minInterSyInt + 1):(bursts(song_idx).start + minInterSyInt))); 
                    end 
...
% Binned data as irina does it 
line2plot = sum(usvmat_tot, 1);
line2plot = mean(reshape(line2plot, 100, []), 1);
line2plot = zscore(line2plot, [], 2); 
figure; 
plot(linspace(-5, 5, 100), line2plot); 
xline(0); yline(0)
ylabel('FR z-score')
xlabel('time (s)'); 
title([BrainArea ' type ' num2str(calltype)])

one = line2plot; 
two = line2plot; 
three = line2plot; 

clearvars -except one two three
...
% title([BrainArea ' ' num2str(calltype)], 'FontWeight','normal') 
% xlim([-4.5 4.5])
% 
% % line profile 
% figure; 
...
% title([BrainArea ' ' num2str(calltype)])
...
% th = line2plot; 
% 
% clearvars -except acc str th
% 
% figure; 
% plot(linspace(-5, 5, 100), acc); hold on; 
% plot(linspace(-5, 5, 100), str);
% plot(linspace(-5, 5, 100), th);
% xline(0); yline(0)
% ylabel('FR z-score')
% xlabel('time (s)'); 
% legend({'acc', 'str', 'th', '', ''})

%% just filter based on call types 
    ...
    if [experiments4mouse.(['target' num2str(find(strcmp({'ACC','Str','TH'}, BrainArea)))])],(1) == 1; % check targetting 
        ...
        for exp_idx = 1 : size(experiments4mouse, 2) 
            ...
            if double(string(Calls.('Type')(1))) == 9 && double(string(Calls.('Type')(end))) == 8  
                ...
                syllables(1, :) = []; % remove the first "call" - artificially added start
                syllables(end, :) = []; % remove the last "ca;;" - artificially added end 
                syllables = syllables(double(string(Calls.Type(2:end-1))) == calltype, :); % filter based on call type; 
            else 
                 disp([experiment.USV ' start or end incorrectly labeled!']); 
            end 
...
figure; 
plot(linspace(-5, 5, 100), one); hold on; 
plot(linspace(-5, 5, 100), two);
plot(linspace(-5, 5, 100), three);
xline(0); yline(0)
ylabel('FR z-score')
xlabel('time (s)'); 
