...
T = readtable('Q:\Personal\Tony\Analysis\USV_csvs\old\ephysUSVfeatures_pooled_tsne.csv', 'Delimiter', ',');
...
for exp_idx = 1 : size(experiments, 2)
    ...
    rec_end = (audiodata.Duration - Calls.Box(1, 1))*1000; % for later last sentence 
    ...
    if size(T_exp, 1) == size(Calls, 1) % check call size agrees 
        ...
        if ~isempty(syllables)
            ...
            while songs(end, 1) + minInterSyInt > rec_end 
                songs(end, :) = []; 
                sentences(end) = []; 
            end 
...
% save it as a df 
writetable(sen_tot, 'Q:\Personal\Tony\Analysis\USV_csvs\old\ephysUSVsentences.csv', 'QuoteStrings', true);
