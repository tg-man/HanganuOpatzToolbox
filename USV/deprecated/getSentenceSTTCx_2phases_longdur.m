function sentenceSTTC = getSentenceSTTCx_2phases_longdur(T, folder4SM, area1, area2, lags, repeat_calc, folder2save)
% Tony July 2025 
...
if repeat_calc == 0 && exist([folder2save area1 area2 '\'  T.mouse{1}, '.mat']) 
    ...
    for sen_idx =  1 : size(T, 1)
        ...
        % set spikese outside period of interest to 0s 
        sm1_baseline = spike_matrix; 
        sm1_baseline(:, [1:(start - 5000) (start - 4001):end]) = 0;
        sm1_peri = spike_matrix; 
        sm1_peri(:, [1:(start - 700) (start + 299):end]) = 0;
        clusters1 = clusters; 
        ...
        % set spikese outside period of interest to 0s 
        sm2_baseline = spike_matrix; 
        sm2_baseline(:, [1:(start - 5000) (start - 4001):end]) = 0;
        sm2_peri = spike_matrix; 
        sm2_peri(:, [1:(start - 700) (start + 299):end]) = 0;
        clusters2 = clusters; 
        clearvars spike_matrix clusters
