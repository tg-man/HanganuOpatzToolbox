%% Looking at effect of opto on USV call clusters 
...
experiments = experiments(logical(opsin));
experiments = experiments(strcmp({experiments.Exp_type}, 'baseline only') | strcmp({experiments.ramp}, optostim));
    ...
    for exp_idx = 1 : size(exp4mouse, 2)
        ...
        elseif strcmp(experiment.Exp_type, 'opto')
            ...
            if size(T_exp, 1) == size(Calls, 1)
                ...
                OptoT.s0(animal_idx) = OptoT.s0(animal_idx) + sum(ismember(round(Calls.Box(:, 1)*1000), stim(1):stim(end)) & (T_exp.tsne_label == 0)); 
                OptoT.s1(animal_idx) = OptoT.s1(animal_idx) + sum(ismember(round(Calls.Box(:, 1)*1000), stim(1):stim(end)) & (T_exp.tsne_label == 1)); 
            end 
...
btot = sum(OptoT.b0) + sum(OptoT.b1); 
stot = sum(OptoT.s0) + sum(OptoT.s1);
% bar plot with percentage 
figure; 
bardata = [sum(OptoT.b0)/btot, sum(OptoT.b1)/btot; sum(OptoT.s0)/stot, sum(OptoT.s1)/stot];
bar(bardata,'stacked')
...
% bar plot with call number
figure; 
bardata = [sum(OptoT.b0), sum(OptoT.b1); sum(OptoT.s0), sum(OptoT.s1)];
bar(bardata,'stacked')
xticklabels({'baseline', 'ramps'}); 
ylabel('# of calls')
title(optostim)
