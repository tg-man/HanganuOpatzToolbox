function RampSTTC = getRampSTTC_ba(experiment, folder4OM, BrainArea1, BrainArea2, lags, repeat_calc, save_data, folder4OSTTC) 
% Tony Oct 2023 
...
if exist([folder4OSTTC BrainArea1 BrainArea2 '\' experiment.name]) && repeat_calc == 0 
    ...
    RampSTTC.lags = lags; 
    RampSTTC.area1 = BrainArea1; 
    RampSTTC.area2 = BrainArea2; 

    if save_data == 1 
        if ~exist([folder4OSTTC BrainArea1 BrainArea2 '\'], 'dir') 
            mkdir([folder4OSTTC BrainArea1 BrainArea2 '\']); 
        end 
        save([folder4OSTTC BrainArea1 BrainArea2 '\' experiment.name], 'RampSTTC'); 
    else 
        disp('data not saved!')
    end 
