function SUAgetStimulationProperties(experiments, save_data, repeatCalc, folder2save)
for n_animal = 1:length(experiments)
    experiment = experiments(n_animal);
    if ~ isempty(experiment.animal_ID) && nnz(~ isnan(experiment.ramp))
        if  repeatCalc == 0 && exist([folder2save, experiment.name, '_StimulationProperties_raw.mat'])
            ...
            [StimTimestamps] = SUAgetStimulationTimeStamps(experiment, save_data, folder2save);
            ...
            if ~isempty(StimulationProperties_raw)
                if save_data == 1
                    save([folder2save, experiment.name, '_StimulationProperties_raw'],'StimulationProperties_raw');
                end
            else
