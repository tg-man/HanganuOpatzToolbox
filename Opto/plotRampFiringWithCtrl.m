function [spikes_tot, allcells, mice] = plotRampFiringWithCtrl(experiments, folder4matrix, folder4ramps, area)
% Tony March 2025
...
for animal_idx = 1 : length(animals)
    ...
    rampspikes = zeros(numel(cells), 10000, size(exp4mouse, 2)*30); 
    ...
    for exp_idx = 1 : size(exp4mouse, 2)
        ...
        rampspikes(locb, :, (30*(exp_idx-1) + 1) : 30*(exp_idx)) = temp; 

        clearvars SUAdata_ramp temp clusters locb
